    ...
    <div class="container mb-4">
        <div class="row">
            ...
            <div class="col-lg-8 pb-5">
                @foreach($fav as $val)
                    @php $wish=\Illuminate\Support\Facades\DB::table('prodocts')->where('id',$val->prodoct_id)->first(); @endphp
                <!-- Item-->
                <div class="cart-item d-md-flex justify-content-between"><span class="remove-item"><i
                                class="fe-icon-x"></i></span>
                    <div class="px-3 my-3"><a class="cart-item-product" href="/prodoct/{{$wish->seo}}">
                            <div class="cart-item-product-thumb"><img src="img/prodoct/{{$wish->pic}}" alt="Product"></div>
                            <div class="cart-item-product-info">
                                <h4 class="cart-item-product-title">{{$wish->name}}</h4>
                                @if($wish->takhfif>0)
                                <div class="text-lg text-body font-weight-medium pb-1">{{number_format($wish->takhfif)}} تومان</div>
                                    @else
                                    <div class="text-lg text-body font-weight-medium pb-1">{{number_format($wish->gheymat)}} تومان</div>
                                 @endif
                            </div>
                        </a></div>
                </div>
